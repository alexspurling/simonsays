...
Elm.Logo = Elm.Logo || {};
Elm.Logo.make = function (_elm) {
   "use strict";
   _elm.Logo = _elm.Logo || {};
   if (_elm.Logo.values) return _elm.Logo.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var colour = F2(function (colour,highlight) {
      var _p0 = colour;
      switch (_p0.ctor)
      {case "Yellow": return _U.eq(highlight,
           HYellow) ? "#ffc229" : "#F0AD00";
         case "Green": return _U.eq(highlight,
           HGreen) ? "#8cfe2f" : "#7FD13B";
         case "Blue": return _U.eq(highlight,
           HBlue) ? "#38d4ff" : "#60B5CC";
         default: return _U.eq(highlight,
   ...
   var logo = function (highlight) {
      ...
      _U.list([A2($Svg.polygon,
              _U.list([$Svg$Attributes.fill(A2(colour,Yellow,highlight))
                      ,$Svg$Attributes.points("161.649,152.782 231.514,82.916 91.783,82.916")]),
              _U.list([]))
              ,A2($Svg.polygon,
              _U.list([$Svg$Attributes.fill(A2(colour,Green,highlight))
              ...
              _U.list([]))
              ,A2($Svg.polygon,
              _U.list([$Svg$Attributes.fill(A2(colour,Blue,highlight))
                      ,$Svg$Attributes.points("323.298,143.724 323.298,0 179.573,0")]),
              _U.list([]))
              ,A2($Svg.polygon,
              _U.list([$Svg$Attributes.fill(A2(colour,Purple,highlight))
   ...
   var None = {ctor: "None"};
   return _elm.Logo.values = {_op: _op
                             ,None: None
                             ,HGreen: HGreen
                             ,HYellow: HYellow
                             ,HPurple: HPurple
...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var svgpanel = $Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                                  ,_0: "width"
                                                  ,_1: "500px"}
                                                 ,{ctor: "_Tuple2",_0: "height",_1: "500px"}]));
   var mainpanel = $Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                                   ,_0: "margin-left"
                                                   ,_1: "20%"}
   ...
   var Highlight = {ctor: "Highlight"};
   var gameLoop = F2(function (time,model) {
      var _p0 = model.state;
      switch (_p0.ctor)
      {case "Start": return _U.update(model,{state: Highlight});
         case "Highlight": var highlightIndex = model.highlightIndex + 1;
           var newState = _U.cmp(highlightIndex,
           $Array.length(model.sequence)) < 0 ? Pause : WaitForInput;
           return _U.update(model,
           {highlightIndex: highlightIndex,state: newState});
         case "Pause": return _U.update(model,{state: Highlight});
         default: return model;}
   });
   var renderModel = function (model) {
      var currentHighlight = _U.eq(model.state,
      Highlight) ? A2($Maybe.withDefault,
      $Logo.None,
      A2($Array.get,
      model.highlightIndex,
      model.sequence)) : $Logo.None;
      return renderBoard(currentHighlight);
   };
   var Start = {ctor: "Start"};
   var defaultState = {sequence: $Array.fromList(_U.list([$Logo.HYellow
                                                         ,$Logo.HBlue
                                                         ,$Logo.HPurple
                                                         ,$Logo.HGreen]))
                      ,highlightIndex: 0
   ...
   var main = A2($Signal.map,renderModel,model);
